...
public abstract class AbstractButtonMixin extends AbstractWidget {
    ...
    public boolean isHovered(){
        if(lastHovered != isHovered && KelUISounds.config.getBoolean("BUTTON.HOVERED", false)){
            ...
            if(isHovered && isActive()) KelUISounds.playSound(SoundStorage.getSound("button.hovered").getSound(), SoundStorage.getSound("button.hovered").getPitch(), SoundStorage.getSound("button.hovered").getVolume());
        }
        return isHovered;
    }
    ...
    public void setFocused(boolean focused){
        if(lastFocused != focused && KelUISounds.config.getBoolean("BUTTON.FOCUSED", false)){
            lastFocused = focused;
            if(focused && isActive()) KelUISounds.playSound(SoundStorage.getSound("button.focused").getSound(), SoundStorage.getSound("button.focused").getPitch(), SoundStorage.getSound("button.focused").getVolume());
        }
